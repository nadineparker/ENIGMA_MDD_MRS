...
export Project_Path=/PATH/TO/DOWNLOADED/FOLDER/ENIGMA_MDD_PRS
...
export MEMORY=$((${NCORES} * 7700)) 	## orig = 8 cores with 7700 memory
...
exec 1>${OUTDIR}/RUN_ENIGMA_MDD_PRS_${Sample_Name}.log 2>&1
...
if [ ${plinkcount} -gt 0 ];
then
	${PLINK} --bfile ${REF_PLINKFILE} --merge-list ${Sample_Name}_merge_list.txt --out ${Sample_Name}_QC_ENIGMA_MDD
else
	cp ${REF_PLINKFILE}.bed ${Sample_Name}_QC_ENIGMA_MDD.bed
	cp ${REF_PLINKFILE}.bim ${Sample_Name}_QC_ENIGMA_MDD.bim
	cp ${REF_PLINKFILE}.fam ${Sample_Name}_QC_ENIGMA_MDD.fam
fi
...
${RSCRIPT} ${Project_Path}/scripts/createBackingFile.R \
	--file-input ${Project_Path}/anc_proj_files/${Sample_Name}_QC_ENIGMA_MDD.bed \
	--file-output ${Project_Path}/${Sample_Name}_tmp/${Sample_Name}_QC_ENIGMA_MDD.rds

# impute missing values
${RSCRIPT} ${Project_Path}/scripts/imputeGenotypes.R \
	--impute-simple mean0 \
	--geno-file-rds ${Project_Path}/${Sample_Name}_tmp/${Sample_Name}_QC_ENIGMA_MDD.rds \
	--cores $NCORES

export fileGenoRDS=${Project_Path}/${Sample_Name}_tmp/${Sample_Name}_QC_ENIGMA_MDD.rds
...
for SUMSTATS in PGC_MDD*GRCh38.sumstats.gz
do
