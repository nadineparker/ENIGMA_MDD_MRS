...
Within the downloaded “ENIGMA_MDD_PRS” directory, you will find 3 potential scripts to run the analyses:
  - ``Singularity_RUN_ENIGMA_MDD_PRS.sh`` – to run the singularity container. Instructions directly below in the “Using the Singularity Container” section.
  -	``Docker_RUN_ENIGMA_MDD_PRS.sh`` – to run the docker container. Skip to instructions in [Using the Docker Container](https://github.com/nadineparker/ENIGMA_MDD_MRS#using-the-docker-container)
  -	``RUN_ENIGMA_MDD_PRS_noContainer.sh`` – to run the analyses without any containers (requires installing software). Skip to instructions in [Not Using a Container](https://github.com/nadineparker/ENIGMA_MDD_MRS#not-using-a-container)

As mentioned above, we recommend using the singularity or docker containers on a linux based server with at least 32GB of RAM and 20GB of free storage space. Below are some steps to perform the analyses with Singularity, Docker, or no container. 
...
2. Navigate to the “ENIGMA_MDD_PRS” directory and open the “Singularity_RUN_ENIGMA_MDD_PRS.sh” script. Add the appropriate information requested at the top of the script and save the edits. Below is a list of the required information:
    ...
    - ``Project_Path``: replace the text “/PATH/TO/DOWNLOADED/FOLDER/ENIGMA_MDD_PRS” with the full path to the ENIGMA_MDD_PRS directory. For example: export Project_DIR=/Users/nadine/Documents/ENIGMA_MDD_PRS
    ...
    - ``MEMORY``: this specifies the amount of memory available per core in MB. REMEMBER, you will need around 32GB of RAM to run the analyses. The default setting is to use 8 cores with ~8GB of memory (64GB of RAM) which takes ~1.5 hours to run (with n =2000 participants).
...
3. Once the above information is added and saved in the ``Singularity_RUN_ENIGMA_MDD_PRS.sh`` script you can run the analyses (a) by batching a job script or (b) running the analyses locally.
    - We provide an example script to batch SLURM jobs (BATCH_MDD_PRS.job). This will need to be augmented for your server. NOTE: Ensure the cores and memory stated in the ``Singularity_RUN_ENIGMA_MDD_PRS.sh`` script does not exceed the requested cores and memory in the slurm job script.
    - You can also run the script locally by opening a terminal, navigating to the ENIGMA_MDD_PRS directory, and entering the following command: 
``
sh Singularity_RUN_ENIGMA_MDD_PRS.sh 
``
or 
``
bash Singularity_RUN_ENIGMA_MDD_PRS.sh 
``
4. Once the analysis is complete see the [Sharing Outputs](https://github.com/nadineparker/ENIGMA_MDD_MRS#4-sharing-outputs) section
...
4. Navigate to the ``ENIGMA_MDD_PRS`` directory and open the ``Docker_RUN_ENIGMA_MDD_PRS.sh`` script. Add the appropriate information requested at the top of the script and save the edits. Below is a list of the required information:
...
5. Once the above information is added and saved in the ``Docker_RUN_ENIGMA_MDD_PRS.sh`` script you can run the analyses (a) by batching a job script or (b) running the analyses locally.
    - We provide an example script to batch SLURM jobs (BATCH_MDD_PRS.job). This will need to be augmented for your server. NOTE: Ensure the cores and memory stated in the ``Docker_RUN_ENIGMA_MDD_PRS.sh`` script does not exceed the requested cores and memory in the slurm job script.
    - You can also run the script locally by opening a terminal, navigating to the ENIGMA_MDD_PRS directory, and entering the following command: 
``
sh Docker_RUN_ENIGMA_MDD_PRS.sh 
``
or 
``
bash Docker_RUN_ENIGMA_MDD_PRS.sh 
``
6. Once the analysis is complete see the [Sharing Outputs](https://github.com/nadineparker/ENIGMA_MDD_MRS#4-sharing-outputs) section
...
2. Navigate to the “ENIGMA_MDD_PRS” directory and open the “RUN_ENIGMA_MDD_PRS_noContainer.sh” script. Add the appropriate information requested at the top of the script and save the edits. Below is a list of the required information:
...
3. Once the above information is added and saved in the ``RUN_ENIGMA_MDD_PRS_noContainer.sh`` script you can run the analyses (a) by batching a job script or (b) running the analyses locally.
    - We provide an example script to batch SLURM jobs (BATCH_MDD_PRS.job). This will need to be augmented for your server. NOTE: Ensure the cores and memory stated in the ``RUN_ENIGMA_MDD_PRS_noContainer.sh`` script does not exceed the requested cores and memory in the slurm job script.
    - You can also run the script locally by opening a terminal, navigating to the ENIGMA_MDD_PRS directory, and entering the following command: 
``
sh RUN_ENIGMA_MDD_PRS_noContainer.sh 
``
or 
``
bash RUN_ENIGMA_MDD_PRS_noContainer.sh 
``
4. Once the analysis is complete see the [Sharing Outputs](https://github.com/nadineparker/ENIGMA_MDD_MRS#4-sharing-outputs) section
...
All outputs will be written to a directory named output_”SampleName”. The list of output files include:
  ...
  -	``RUN_ENIGMA_MDD_PRS_”Sample_Name”.log`` – a log file for the entire analyses
  -	``“Sample_Name”_PRS.txt`` – a file containing MDD polygenic scores
...
We ask that you share all files in this directory along with the **[Additional Variables](https://github.com/nadineparker/ENIGMA_MDD_MRS#additional-variables)** and the standard **ENIGMA FreeSurfer cortical (SurfAvg/ThickAvg.csv) and subcortical (LandRvolumes.csv) measures**. If you already have imaging and covariate data stored on the centralized server and have granted access to the project analysts in the sign-up form there is no need to re-share this data. 
...
Please use NA for each participant if one of the variables below is not included in your sample.
 ...
 - ``Dx`` - Diagnosis coded as patient = MDD and control = CN
...
- ``Stage`` - Stage of illness coded as primary, recurrent, or remission (if available).
- ``DiseaseDuration`` - Years since diagnosis (if available).
- ``DepressiveSymptoms`` - Current symptoms Yes or No (if available).
- ``HDRS`` - Hamilton Depression Rating Scale score (if available).
- ``BDI`` - Beck's Depression Inventory score (if available). 
- ``MedicationUse`` - Yes or No (if available).
- ``Antidepressant`` - Antidepressant usage 0=controls, 1=patients without antidepressant use, 2= patients with antidepressant use
- ``Benzodiazepine`` - Benzodiazepine usage 0=controls, 1=patients without benzodiazepine use, 2= patients with benzodiazepine use
- ``MoodStabalizer`` - Mood stabalizing usage 0=controls,1=patients without mood stabilizer use 2= patients with mood stabilizer use
- ``Antipsychotic`` - Antipsychotic usage 0=controls,1=patients without antipsychotic use 2= patients with antipsychotic use
...
MAC (M1/M2): docker pull -–platform=linux/amd64 ghcr.io/comorment/ldpred2:latest
```
2. Navigate to the “ENIGMA_MDD_PRS” project directory and depending on which container you choose open the “Auto_LiftOver_Singularity.sh” or “Auto_LiftOver_Docker.sh” script. Add the appropriate information requested at the top of the script and save the edits. Below is a list of the required information:
    - ``Base_Dir``: replace the text “/PATH/TO/BASE/DIR” with the full path to a parent directory that contains all the necessary files and downloaded project directory. For example: export Base_Dir=/Users/nadine
    - ``Project_Path``: replace the text “/PATH/TO/DOWNLOADED/FOLDER/ENIGMA_MDD_PRS” with the full path to the ENIGMA_BD_PRS directory. For example: export Project_DIR=/Users/nadine/Documents/ENIGMA_MDD_PRS
    - ``Sample_Dir``: replace the text “/PATH/TO/GENETIC/DATA” with the full path to your cohorts PLINK v1 files (NOTE, .bed, .bim, .fam should all be in the same directory). For example: export Sample_Dir=/Users/nadine/Documents
    - ``Prefix``: add the prefix used for the PLINK v1 files (.bed, .bim, .fam). For example: export Prefix=TOP_GRCh38
